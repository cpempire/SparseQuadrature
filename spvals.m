function z = spvals(f, d, range, options, data, varargin)
...
gridType = spget(options,'GridType','gauss-hermite');
zprev = spget(options, 'PrevResults');
if strcmpi(dimadaptive, 'on')
% 	z = spadaptvals(f, d, range, options, data, varargin{:});
%     if strcmp(lower(adaptivestyle),'onebyone')
%         z = spadaptquad_dimadapt_dstep(f, d, range, options, data, varargin{:});
%     else
%         z = spadaptquad(f, d, range, options, data, varargin{:});
%     end

    if strcmpi(gridType,'gauss-hermite') || strcmpi(gridType,'kronrod-patterson-normal')
        z = spadaptquad_dimadapt_dstep(f, d, range, options, data, varargin{:});
    else
        z = spadaptvals(f, d, range, options, data, varargin{:});
    end
