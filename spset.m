...
if (nargin == 0) && (nargout == 0)
  ...
  fprintf('  DistributionType: [ {uniform} | beta | normal ]\n');
  fprintf('       ProblemType: [ {interpolation} | integration ]\n');               
  fprintf('      Verification: [ {off} | on]\n');
  fprintf('        KeepWeight: [ {off} | on ]\n');	
  ...
  fprintf('     AdaptiveStyle: [ {full} | onebyone ]\n');	
  ...
  fprintf('       GrowthLevel: [ {double} | one ]\n')
  fprintf('\n');
  return;
end

Names = {'GridType', 'ProblemType', 'DistributionType', 'KeepWeight', 'Verification', ...
                 'RelTol', 'AbsTol', 'Vectorized', 'MinDepth', ...
				 'MaxDepth', 'VariablePositions', 'NumberOfOutputs', ...
				 'PrevResults', 'FunctionArgType', 'KeepFunctionValues', ...
				 'KeepGrid', 'DimensionAdaptive', 'AdaptiveStyle', 'MinPoints', ...
				 'MaxPoints', 'DimadaptDegree', 'DegreeStrategy', ...
				 'SparseIndices', 'DropTol', 'EnableDCT','GrowthLevel'};
