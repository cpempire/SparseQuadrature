function ip = barypdstepkpn(z, allnx, dims, x, y, w)
% BARYPDSTEPGP  Step of Barycentric d-dim. Lagrange interpolation 
%    Kronrod-Patterson-Normal grid. Includes additional weight vector w
...
% record the index of new nodes in the current level
for actna = [1,2,6,10,16]
    switch actna 
      case 1 
          ik{1} = uint32(1);
      case 2
          ik{2} = uint32([1,3]);
      case 6
          ik{6} = uint32([1,2,4,6,8,9]);
      case 10
          ik{10} = uint32([1,2,4,6,8,12,14,16,18,19]);
      case 16
          ik{16} = uint32([1,2,3,5,7,9,13,17,19,23,27,29,31,33,34,35]);
      otherwise
  ...
  for l = 1:d
    ...
    if actnx == 3
      ...
      s = s + frac;  
        ...
        % check overlapping point
        k = 0;
        for idx = xid+1:xid+actnx
            if t == x(idx)
                k = idx-xid;
            end
        end
      ...
      % do every element for denominator
      for kxi = xid+1:xid+actnx
          frac = w(kxi)/(t-x(kxi));
          s = s + frac;
      end
      % do every element for numerator
      for k = aid+1:aid+actna
          a(k) = w(xid+ik{actna}(k-aid))/(t-x(xid+ik{actna}(k-aid)));
      end
    end
    c2(l) = s;
    aid = aid+actna;
