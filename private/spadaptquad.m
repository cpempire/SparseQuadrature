function z = spadaptquad(f, d, range, options, data, varargin)
...
while (ntotalpoints < maxPoints) && ~success
    ...
    if ni > niold
        ...
        while kend < ni
            ...
            for k = kstart:kend
                % record error indicator at current index
    ...
    if ni > niold && (ntotalpoints >= minPoints || all(maxlevel == maxDepth)) 
       ...
       if strcmpi(problemType, 'integration')
           ...
           error('this code is only for integration purpose')
       end
...
%         points = [points, ntotalpoints];
        ...
        if strcmpi(problemType, 'integration')
            ...
            for l = 1:nresults
                ...
                Eest{l} = [Eest{1}, sum(Emeank(l,kstart:kend))];
            end    
        end    
%     end
%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%           
