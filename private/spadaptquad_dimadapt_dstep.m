function z = spadaptquad_dimadapt_dstep(f, d, range, options, data, varargin)
...
growthLevel = spget(options, 'GrowthLevel','one');
...
elseif strcmpi(gridtype, 'gauss-hermite') && strcmpi(growthLevel,'double')   
  nmax = spget(options, 'MaxDepth', 6);
	if nmax > 6
		warning('MATLAB:spinterp:badopt',['Maximum supported depth ' ...
		  'level for the gauss-hermite grid with double is 6, but MaxDepth ' ...
			'was set to ' num2str(nmax) '. Using MaxDepth = 6 instead.']);
		nmax = 6;
    end    
else
...
val = posvars(dtotal, varpos, varargin{:});
...
if isempty(zprev)
  ...
  z = cell(nresults,1);
  [z(1:nresults), x] = ...
      spevalf_simple(gridgen, f, levelseq, d, ...
        ...
        if isadmissible
              ...
              if nbid == 0 || (i > dold)
                    ...
                    while j <= nbidtemp
                          ...
                          elseif isgh == 1
                              if strcmpi(growthLevel,'double')
                                 npoints = npoints * (2^uint32(lev+1)-1);
                              else
    ...
    if ni > niold
        ...
        while kend < ni
			...
			[znew(1:nresults), xnew] = ...
					spevalf_simple(gridgen, f, levelseq, d, [kstart, kend], ...
									range, varpos, vectorized, nresults, ...
									functionArgType,data,val{:});
            ...
            for l = 1:nresults
                ...
                % Emeank(l,(Emeank(l,:)/max(Mean{l}(end),eps) < 1e-15)) = 0;
            end
            surplusCompTime = surplusCompTime + etime(clock,t0);
            
            for k = kstart:kend      
                %npoints = ptslen(k);
				G(k) = max(abs(Emeank(:,k)));%/double(npoints);
...
%         points = [points, ntotalpoints];
        ...
        if strcmpi(problemType, 'integration')
            ...
            for l = 1:nresults
                ...
                Emean{l} = [Emean{l}, sum(EmeankActive)];
