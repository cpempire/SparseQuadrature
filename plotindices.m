...
switch d
 case 2
	...
	plot(indices(:,1)+1,indices(:,2)+1,'kx','LineWidth',1, 'MarkerSize', 20);
	hold on;
	activeIndices = find(z.indices.active);
	plot(indices(activeIndices,1)+1, ...
			 indices(activeIndices,2)+1,'bs','LineWidth',1, 'MarkerSize', 20);
...
% 	axis tight;
	...
	set(gca,'XTick', 1:xlim(2), 'FontSize', 16);
	set(gca,'YTick', 1:ylim(2), 'FontSize', 16);
	set(gca,'YDir','normal');
 otherwise
