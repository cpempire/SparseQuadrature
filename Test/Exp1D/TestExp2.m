...
accuracy_kpn = [1,3,8,15,25]; %25% order of accuracy 
accuracy_ghn = 1:63;%[1,3,7,15,31,63]; % # points 
...
%     subplot(1,2,2)
    ...
    set(gca,'FontSize',16)
    saveas(gca,'QuadExp2D1','epsc')
end

accuracy_kpn = [1,3,8,15,25]; % order of accuracy 
accuracy_ghn = 1:15;%[1,3,7,15,31,63]; % # points 
for dim = 8
    clear value_u mean_u node_u
    for ia = 1:length(accuracy_kpu)
        [x, w] = nwspgr('KPU', 1, accuracy_kpu(ia));
...
%     subplot(1,2,2)
    ...
    semilogy(node_u(1:end),abs(mean_u(1:end) - mean_g(end)),'b*-')
    semilogy(node_n(1:end),abs(mean_n(1:end) - mean_g(end)),'r.-','MarkerSize',10)
%     legend('GH1:e^{-2y^2}','tGKP:e^{-2y^2}','GK:e^{-2y^2}', ...
%            'GH1:e^{-y^2/2}','tGKP:e^{-y^2/2}','GK:e^{-y^2/2}')
    legend('GH1','tGKP','GK')
    xlabel('$m_l$','Interpreter','latex')
    ylabel('$|I(f) - \mathcal{Q}_l(f)|$','Interpreter','latex')
    set(gca,'FontSize',16)
    saveas(gca,'QuadExp2D2.eps','epsc')
end